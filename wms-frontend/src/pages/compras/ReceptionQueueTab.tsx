...
export default function ReceptionQueueTab({ onRefresh }: { onRefresh: () => void }) {
  ...
  useEffect(() => {
    api.get('/purchasing/reception-queue')
  ...
  const handlePartialReceipt = async (oc: any) => {
    ...
    onRefresh();
    // Reload
    const r = await api.get('/purchasing/reception-queue');
    setQueue(r.data || []);
  };
  ...
  if (loading) return <div className="text-center text-white/30 py-12">Cargando cola de recepción...</div>;
          ...
          <div key={oc.id} className="bg-primary-800/30 border border-white/5 rounded-xl p-5 hover:border-white/10 transition-all">
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                {oc.estado === 'PARCIAL' && (
                  <span className="px-2.5 py-1 bg-orange-500/20 text-orange-400 rounded-full text-xs font-medium">PARCIAL</span>
                )}
            ...
            <div className="mb-3">
              ...
              <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${oc.porcentajeRecibido >= 100 ? 'bg-emerald-500' : oc.porcentajeRecibido > 0 ? 'bg-amber-500' : 'bg-white/10'}`}
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {oc.lineas?.map((l: any) => (
                <div key={l.id} className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg text-xs">
                  ...
                  <span className="text-white/30">|</span>
                  <span className="text-white/50">{l.rollosRecibidos || 0} rollos</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-xs text-white/40">
                <Clock size={14} />
                {eta ? (
                  <span>ETA: {eta.toLocaleDateString('es-MX')} ({daysToEta !== null && daysToEta > 0 ? `en ${daysToEta} días` : daysToEta === 0 ? 'hoy' : 'atrasado'})</span>
                ) : <span>Sin ETA</span>}
              </div>
              <div className="flex gap-2">
                <button
                  onClick={() => handlePartialReceipt(oc)}
