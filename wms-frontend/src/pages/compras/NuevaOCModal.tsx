...
import { X, Plus, Trash2, Search, Tag, Info } from 'lucide-react';
...
export default function NuevaOCModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => {
    api.get('/catalog/suppliers', { params: { limit: 100 } }).then(r => setSuppliers(r.data.data || r.data || []));
    api.get('/catalog/skus', { params: { limit: 100 } }).then(r => setSkus(r.data.data || r.data || []));
  }, []);
  ...
  const addSku = async (sku: any) => {
    ...
    setLineas([...lineas, {
      skuId: sku.id,
      skuLabel: `${sku.codigo} — ${sku.nombre}`,
      cantidadRollos: rollos,
      metrajePorRollo: metraje,
      metrajeTotal: rollos * metraje,
      precioUnitario: precio,
      precioFuente: fuente,
      importe: rollos * metraje * precio,
    }]);
    setSkuSearch('');
  };
  ...
  const updateLine = (idx: number, field: string, value: number) => {
    setLineas(prev => prev.map((l, i) => {
      ...
      if (field === 'cantidadRollos' || field === 'metrajePorRollo') {
        updated.metrajeTotal = updated.cantidadRollos * updated.metrajePorRollo;
      }
    ...
    try {
      await api.post('/purchasing/orders', {
        ...
        lineas: lineas.map(l => ({
          ...
          metrajePorRollo: l.metrajePorRollo,
      ...
      alert(e?.response?.data?.message || 'Error al crear OC');
    }
    setSaving(false);
  };
      ...
      <div className="bg-primary-900 border border-white/10 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-primary-800/50 border border-white/10 rounded-xl text-white text-sm focus:border-blue-500/50 focus:outline-none"
              />
            </div>
                        ...
                        return (
                          <tr key={i} className="border-b border-white/5">
                            ...
                            <td className="px-3 py-2">
                              <input type="number" min={1} value={l.cantidadRollos} onChange={e => updateLine(i, 'cantidadRollos', parseInt(e.target.value) || 1)}
                                className="w-16 px-2 py-1 bg-white/5 border border-white/10 rounded text-white text-xs text-center" />
                            </td>
                            <td className="px-3 py-2">
                              <input type="number" min={1} value={l.metrajePorRollo} onChange={e => updateLine(i, 'metrajePorRollo', parseFloat(e.target.value) || 50)}
                                className="w-16 px-2 py-1 bg-white/5 border border-white/10 rounded text-white text-xs text-center" />
                            </td>
                            <td className="px-3 py-2 text-white/60 text-xs">{l.metrajeTotal}m</td>
                            <td className="px-3 py-2">
                              <input type="number" step="0.01" value={l.precioUnitario} onChange={e => updateLine(i, 'precioUnitario', parseFloat(e.target.value) || 0)}
                                className="w-20 px-2 py-1 bg-white/5 border border-white/10 rounded text-white text-xs text-center" />
                            </td>
                            <td className="px-3 py-2">
                              <span className={`text-xs ${f.color}`}>{f.text}</span>
                            </td>
                            <td className="px-3 py-2 text-white font-medium text-xs">${l.importe.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                            <td className="px-3 py-2">
                              <button onClick={() => removeLine(i)} className="text-red-400 hover:text-red-300"><Trash2 size={14} /></button>
                            </td>
                          </tr>
                        );
                      })}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select value={prioridad} onChange={e => setPrioridad(Number(e.target.value))}
                  className="w-full px-3 py-2 bg-primary-800/50 border border-white/10 rounded-xl text-white text-sm">
              ...
              <div>
                <label className="text-xs text-white/50 mb-1 block">Fecha estimada entrega</label>
              ...
              <div>
                ...
                <input value={condiciones} onChange={e => setCondiciones(e.target.value)} placeholder="Ej: Crédito 30 días"
                  className="w-full px-3 py-2 bg-primary-800/50 border border-white/10 rounded-xl text-white text-sm placeholder:text-white/30" />
              </div>
              <div>
        ...
        <div className="flex justify-end gap-3 p-5 border-t border-white/10">
          ...
          <button
            ...
            disabled={!supplierId || lineas.length === 0 || saving}
